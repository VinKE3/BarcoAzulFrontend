...
import { BloquearCompraFilter, useBloquearCompraColumn } from ".";
import {
  ButtonGroup,
  DeleteModal,
  Loading,
...
import {
  defaultBloquearCompra,
  IBloquearCompra,
  IBloquearCompraTablas,
...
const BloquearCompra: React.FC = () => {
  //#region useState
  const navigate = useNavigate();
  const menu: string = "Compra/BloquearCompra";
  const menuTabla: string = "Compra/BloquearCompra/FormularioTablas";
  ...
  const { visible, permisos } = usePermisos("BloquearCompra");
  ...
  const handleModal = async (): Promise<void> => {
    ...
    handleInitialData(globalContext, defaultBloquearCompra)
      .then((response) => {
        const {
          data,
          tablas,
        }: { data: IBloquearCompra; tablas: IBloquearCompraTablas } = response;
  ...
  const bloquearCompra = async (): Promise<void> => {
    try {
      const { row }: { origen: string; row: IBloquearCompraTable } = retorno;
  ...
  const columns = useBloquearCompraColumn(api.loading, setGlobalContext);
  ...
  return (
    <div className="main-base">
      <div className="main-header">
        <h4 className="main-header-title">Bloquear Compras</h4>
        {ready && visible && (
      ...
      <div className="main-body">
        ...
        {ready && visible && (
          <>
            ...
            {visible && <BloquearCompraFilter tablas={tiposDocumento} />}
            {visible && (
              <Table
                data={table.data}
                columns={columns}
                isTablas={true}
                tableClassName="bloquear-compra-table"
              />
            )}
