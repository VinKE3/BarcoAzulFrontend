...
import { DocumentoVentaFilter, useDocumentoVentaColumn } from ".";
import {
  ButtonGroup,
  DeleteModal,
  Loading,
...
const DocumentoVenta: React.FC = () => {
  ...
  const { primer } = modal;
  const { visible, permisos } = usePermisos("DocumentoVenta");
  const mensaje = mensajes.filter((x) => x.origen === "global" && x.tipo >= 0);
  const [ready, setReady] = useState(false);
  const columns = useDocumentoVentaColumn();
  //#endregion
  ...
  const handleModal = async (): Promise<void> => {
    ...
    handleInitialData(globalContext, defaultDocumentoVenta)
      .then((response) => {
        const {
          data,
          tablas,
          ...
          <>
            ...
            {visible && (
              <Table
                ...
                isPermitido={true}
                isTablas={true}
                tableClassName="documento-venta-table"
              />
            )}
