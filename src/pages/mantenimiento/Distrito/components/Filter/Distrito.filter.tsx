...
import { IDistritoFilter, IDistritoTable, defaultDistritoFilter } from "../../../../../models";
import { getListar, handleInputType, handleSetErrorMensaje, resetPagination } from "../../../../../util";

const DistritoFilter: React.FC = () => {
  //#region useState
  ...
  const [filter, setFilter] = useState<IDistritoFilter>(defaultDistritoFilter);
  ...
  const handleListar = async (): Promise<void> => {
    try {
      ...
      const { data, total }: { data: IDistritoTable[]; total: number } = await getListar(globalContext, params);
