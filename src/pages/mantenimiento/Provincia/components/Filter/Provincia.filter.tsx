...
import { IProvinciaFilter, IProvinciaTable, defaultProvinciaFilter } from "../../../../../models";
import { getListar, handleInputType, handleSetErrorMensaje, resetPagination } from "../../../../../util";

const ProvinciaFilter: React.FC = () => {
  //#region useState
  ...
  const [filter, setFilter] = useState<IProvinciaFilter>(defaultProvinciaFilter);
  ...
  const handleListar = async (): Promise<void> => {
    try {
      ...
      const { data, total }: { data: IProvinciaTable[]; total: number } = await getListar(globalContext, params);
...
export default ProvinciaFilter;
