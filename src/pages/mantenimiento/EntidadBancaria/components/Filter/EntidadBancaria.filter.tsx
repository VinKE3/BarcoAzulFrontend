...
import { IEntidadBancariaFilter, IEntidadBancariaTable, defaultEntidadBancariaFilter } from "../../../../../models";
import { getListar, handleInputType, handleSetErrorMensaje, resetPagination } from "../../../../../util";

const EntidadBancariaFilter: React.FC = () => {
  //#region useState
  ...
  const [filter, setFilter] = useState<IEntidadBancariaFilter>(defaultEntidadBancariaFilter);
  ...
  const handleListar = async (): Promise<void> => {
    try {
      ...
      const { data, total }: { data: IEntidadBancariaTable[]; total: number } = await getListar(globalContext, params);
      setGlobalContext((x) => ({
        ...x,
...
export default EntidadBancariaFilter;
